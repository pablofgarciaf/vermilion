...
- **Coherencia NAP Estricta (Nombre, Dirección, Teléfonos):** En Schema (`layout.tsx`), registrar ambos teléfonos de la empresa en `contactPoint` (Atención al cliente / WhatsApp `+593 96 003 9156` y Reservas / Oficina `+593 99 404 8458`) para mantener paridad con Google Business Profile y TripAdvisor.
...
### 10. ESTÁNDAR GEO AVANZADO: CÁPSULAS DE RESPUESTA DIRECTA Y TRANSPARENCIA DE TARIFAS
- **Cápsulas de Respuesta Directa (Direct Answer / Key Takeaways):** Todo artículo editorial o guía de destino debe incluir una cápsula semántica `<aside aria-label="Quick Summary">` en los primeros 1,000 caracteres de lectura del DOM. Esta cápsula debe responder de forma concisa y directa (2-3 oraciones) a la intención de búsqueda principal, acompañada de datos duros estructurados: mejor época, duración recomendada, dificultad física, precio orientativo y punto culminante. Esto garantiza que ChatGPT, Perplexity, Gemini y Google AI Overviews recuperen a Vermilion Routes como fuente canónica.
- **Esquema Dual `BlogPosting` + `FAQPage`:** Cada artículo debe incorporar preguntas frecuentes (FAQs) con respuestas directas e inyectar el esquema JSON-LD dual en Server Components para enriquecer la visibilidad en Rich Results.
- **Transparencia de Precios "Desde $X USD" (Starting Price):** Para evitar rebote y superar auditorías automatizadas de IA que verifican costos antes de recomendar un itinerario, toda tarjeta de destino o tour debe mostrar de forma explícita el prefijo localizado de precio inicial (`Desde $X USD` / `From $X USD`).
### 11. ESTÁNDAR E-E-A-T DE AUTORÍA FIDUCIARIA & CONFIANZA INSTITUCIONAL
- **Ficha de Autor Real Verificable:** Todo contenido editorial o guía turística debe estar respaldado por una ficha de autor completa (`BlogAuthorBio.tsx`), con biografía, fotografía, rol institucional (`Lead Naturalist & Head of Expedition Design`), años de experiencia y credenciales oficiales del Ministerio de Turismo del Ecuador (`Reg. 1793215456001`).
- **Esquema `Person` + `hasCredential` Enriquecido:** El JSON-LD de `BlogPosting` debe incorporar el objeto `author` con `@type: "Person"`, `jobTitle`, `worksFor` (`Vermilion Routes`) y credenciales reconocidas por el Ministerio de Turismo. Prohibido publicar artículos con autores anónimos o sin respaldo curricular.
- **Erradicación de Meta Keywords Obsoletas:** No emitir etiquetas `<meta name="keywords">` sobrecargadas en el `<head>`, ya que señalan prácticas SEO anticuadas y exponen la estrategia a competidores sin beneficio para el posicionamiento orgánico moderno.

### 12. AUDITORÍA CRÍTICA DE PROVEEDORES DE IA (BISTURÍ TÉCNICO VS ALUCINACIÓN)
- **Protocolo de Validación de Auditorías Externas:** Al recibir auditorías de modelos externos (Claude, DeepSeek, GPT):
  - **Identificar Auditorías Fidedignas (Estilo Claude / DeepSeek):** Se reconocen porque citan cadenas de texto literales del HTML servido (ej. `"Loading travel guides..."`), identifican discrepancias físicas de NAP verificables en Google/TripAdvisor y señalan faltantes semánticos accionables sin inventar componentes.
  - **Descartar y Neutralizar Auditorías Alucinantes (Estilo GPT / Retórica Pura):** Se caracterizan por jerga pomposa ("metamorfosis tectónica", "estrato HNWI") e invención flagrante de errores inexistentes (como "carruseles 3D interactivos", "títulos con paréntesis vacíos ()", o alegar que no hay `hreflang` ni `robots.txt` cuando están 100% implementados). Prohibido modificar código basándose en quejas ficticias de modelos alucinantes.
- **Cross-Linking Transaccional Obligatorio:** Ningún artículo del blog puede ser un callejón sin salida (dead-end). Todo post debe enlazar bidireccionalmente a la expedición recomendada vía `BlogTourBookingShowcase.tsx` y hacia el archivo fotográfico de destino.
