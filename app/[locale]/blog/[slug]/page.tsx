...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#0B1622] text-zinc-900 dark:text-zinc-100 pt-8 sm:pt-10 pb-20 px-4 sm:px-6 lg:px-8 font-sans selection:bg-emerald-500 selection:text-white transition-colors duration-300">
      <script
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        {/* Back Link (discreto, integrado) */}
        <Link
          href={`/${locale}/blog`}
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 dark:text-zinc-400 hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors"
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>{backLabel}</span>
        </Link>

        {/* Article Header: imagen a la izquierda, texto a la derecha en desktop */}
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          {/* Featured Image */}
          <div className="relative h-64 sm:h-80 lg:h-[420px] w-full rounded-3xl overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800 order-1">
            <Image
              src={post.imageUrl}
          ...
          <div className="space-y-4 order-2">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-[2.75rem] font-bold font-serif text-zinc-900 dark:text-white leading-tight tracking-tight">
            ...
            <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-emerald-600 dark:border-emerald-700 shrink-0">
                  <Image quality={85} src={post.author.avatar} alt={post.author.name} width={40} height={40} className="w-full h-full object-cover" />
                </div>
                <div>
