   ...
   for (const doc of snapshot.docs) {
      ...
      const fixPath = (path) => {
         ...
         if (!existsSync(localPath)) {
            // Try 16-9 version
            let alt = path.replace('/9-16/', '/16-9/').replace('-9-16.webp', '-16-9.webp');
            if (!alt.endsWith('.webp')) alt = alt.replace(/\.\w+$/, '.webp');
            if (path.endsWith('.webp') && !alt.endsWith('.webp')) alt = alt + '.webp';
